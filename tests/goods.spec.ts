...
describe("Pruebas de Goods", () => {
  ...
  test("Prueba básica de inicialización 2"), () => {
    expect(good1.id).toBe(2);
    expect(good1.name).toBe("Colonia de Lirio y Grosellas");
    expect(good1.description).toBe("Una colonia muy especial");
    expect(good1.material).toBe("Lirios y grosellas");
    expect(good1.weight).toBe(0.7);
    expect(good1.value).toBe(60);
  }
});